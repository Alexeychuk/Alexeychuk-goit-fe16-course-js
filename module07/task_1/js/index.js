...
const getTotalBalance = users => users.reduce((totalBalance, user) => {
...
// const getNamesSortedByFriendsCount = users => users.reduce((sortArr,user) => {
//     user.friends.length  
// },[]);

const sortByLength = (a, b) => a.friends.length - b.friends.length;

const getNamesSortedByFriendsCount = users => users.sort(sortByLength).map(user => user.name);
  // твой код
